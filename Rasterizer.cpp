...
vector<Vertex> Rasterizer::FillBetweenVerticies(const Vector3& vec, const Rectangle& rec, const float& totalArea, const Vertex& v1, const Vertex& v2, const Vertex& v3)
{
    vector<Vertex> verticies;
    for (float x = rec.x; x < rec.x + rec.width; x++)
    {
        auto point = Math::Vector2(x, rec.y);
        ...
        if (area <= totalArea + 0.5f && area >= totalArea - 0.5f)
        {
            ...
            verticies.push_back(Vertex({ x, rec.y }, c));
        }
    }
    return verticies;
}
   ...
   /* float base = std::max(std::max(a, b), c);
    ...
    if (mode == FillMode::Solid)
    {
        ...
        for (float y = rec.y; y < rec.y + rec.height; y++)
            futures.push_back(async(std::launch::async, FillBetweenVerticies, Vector3(a, b, c), rec, totalArea, v1, v2, v3));
